...
function summarizePlanFeatures(features: PublicPlanFeature[]): string[] {
  ...
  // Any other enabled admin features not in a group still appear individually.
  ...
  useEffect(() => {
    void fetch(`${API_URL}/api/v1/public/site-settings`)
      .then(async (res) => {
        ...
        if (data.plansPageEnabled === false) {
          setAllowed(false);
    ...
    return (
      <section className="ui-mkt__band ui-mkt__band--plans">
        <div className="ui-mkt__section">
          <LoadingState>{allowed === false ? "Redirecting to contact…" : "Loading…"}</LoadingState>
        </div>
      </section>
    );
  }
