...
export default function AnalyticsPage() {
  ...
  async function load(selected = period) {
    ...
    const [overviewData, financialData, clientData, activityData] = await Promise.all([
      api<Record<string, number>>(`/api/v1/dietitian/${dietitianAccountId}/analytics/overview${query}`),
      api<Financial>(`/api/v1/dietitian/${dietitianAccountId}/analytics/financial${query}`),
      api<{
        needsAttention: Array<{ clientId: string; clientName: string; clientEmail?: string | null; reasons: string[] }>;
      }>(`/api/v1/dietitian/${dietitianAccountId}/analytics/clients${query}`),
      api<Record<string, number>>(`/api/v1/dietitian/${dietitianAccountId}/analytics/activity${query}`),
    ]);
