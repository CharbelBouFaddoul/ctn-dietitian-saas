...
export default function CalendarPage() {
  ...
  useEffect(() => {
    void Promise.all([
      api<AppointmentRow[]>(`/api/v1/dietitian/${dietitianAccountId}/appointments`),
      api<{ items: TaskRow[] }>(`/api/v1/dietitian/${dietitianAccountId}/tasks?limit=100`),
    ])
      .then(([appointments, tasks]) => {
        const appointmentItems: CalendarItem[] = appointments.map((row) => ({
          ...
          href: `/practice/${dietitianAccountId}/clients/${row.client.id}`,
        }));
        const taskItems: CalendarItem[] = tasks.items
          ...
          .map((row) => ({
            ...
            href: row.clientId
              ? `/practice/${dietitianAccountId}/clients/${row.clientId}`
              : `/practice/${dietitianAccountId}/tasks`,
          }));
        setItems(
