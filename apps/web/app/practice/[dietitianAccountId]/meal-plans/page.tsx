...
function MealPlansPage() {
  ...
  const [sourcePlans, setSourcePlans] = useState<PlanRow[]>([]);
  ...
  async function loadSourcePlans(forClientId: string) {
    if (!forClientId) {
      setSourcePlans([]);
      setSourcePlanId("");
      return;
    ...
    try {
      const listed = await api<{ items: PlanRow[] }>(
        `/api/v1/dietitian/${dietitianAccountId}/meal-plans?clientId=${encodeURIComponent(forClientId)}&pageSize=50`,
      );
      const items = listed.items.filter((row) => row.status !== "ARCHIVED");
      setSourcePlans(items);
      setSourcePlanId((current) => (items.some((row) => row.id === current) ? current : (items[0]?.id ?? "")));
    } catch (err) {
      setSourcePlans([]);
      setSourcePlanId("");
      setError(errorMessage(err, "Could not load plans to import"));
    } finally {
  ...
  function openCreate() {
    setShowCreate(true);
    setStartMode("blank");
    const fromFilter = filterClientId && clients.some((row) => row.id === filterClientId);
    const nextSource = fromFilter ? filterClientId : (clientId || clients[0]?.id || "");
    setSourceClientId(nextSource);
    void loadSourcePlans(nextSource);
  }
      ...
      const created = await api<{ id: string; versions: Array<{ id: string; status: string }> }>(
        `/api/v1/dietitian/${dietitianAccountId}/meal-plans`,
        {
          ...
          body: JSON.stringify({
            ...
            dayLabelMode,
            ...(startMode === "import" && sourcePlanId ? { sourcePlanId } : {}),
          }),
        },
      );
    ...
    <section className="ui-list-page">
      ...
      {showCreate ? (
        <Section
          ...
          description={
            startMode === "import"
              ? "Copies every week, day, meal, and food from an existing plan into a new draft."
              : "Starts as a draft with one day and Breakfast / Lunch / Dinner."
          }
        >
          <form onSubmit={(event) => void create(event)} className="ui-stack" style={{ gap: 16, width: "100%" }}>
            ...
            <div>
              ...
              <div className="ui-mp__choice">
                ...
                >
                  <strong>Blank</strong>
                  <span>Empty day with Breakfast / Lunch / Dinner</span>
                </button>
                <button
                  type="button"
                  ...
                  onClick={() => {
                    setStartMode("import");
                    if (!sourcePlans.length && !sourceLoading) void loadSourcePlans(sourceClientId || clientId);
                  }}
                >
            ...
            {startMode === "import" ? (
              <>
                ...
                <Field
                  label="Meal plan"
                  hint={sourceLoading ? "Loading…" : "All days and meals are copied. The new plan stays a draft."}
                >
                  <Select value={sourcePlanId} onChange={(event) => setSourcePlanId(event.target.value)} required>
                    {sourcePlans.length === 0 ? (
                      <option value="">{sourceLoading ? "Loading plans…" : "No plans for this client"}</option>
                    ) : null}
            ...
            <div className="ui-row" style={{ gap: 10 }}>
              <Button type="submit" disabled={busy || !clientId || (startMode === "import" && !sourcePlanId)}>
                {busy ? (startMode === "import" ? "Importing…" : "Creating…") : startMode === "import" ? "Import plan" : "Create draft"}
              </Button>
