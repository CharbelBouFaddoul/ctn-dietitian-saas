...
export default function MealPlanEditorPage() {
  ...
  async function searchFoods() {
    ...
    try {
      const result = await api<{ items: Array<{ id: string; name: string }> }>(
        `/api/v1/dietitian/${dietitianAccountId}/foods?q=${encodeURIComponent(foodQuery)}&pageSize=5`,
      );
  ...
  async function searchRecipes() {
    ...
    try {
      const result = await api<{ items: Array<{ id: string; name: string }> }>(
        `/api/v1/dietitian/${dietitianAccountId}/recipes?q=${encodeURIComponent(recipeQuery)}&pageSize=5`,
      );
