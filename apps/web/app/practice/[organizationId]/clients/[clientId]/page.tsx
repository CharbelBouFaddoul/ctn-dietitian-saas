...
function ClientWorkspacePage() {
  ...
  function selectTab(next: string) {
    ...
    setTab(value);
    router.replace(`/practice/${organizationId}/clients/${clientId}?tab=${value}`, { scroll: false });
  }

  async function load() {
    setError(null);
    try {
      const [detail, account, profileRow, goalRows, measurementRows, assessmentRows, appointmentRows, timelineRows, tagRows, templateRows, planRows] =
        await Promise.all([
  ...
  return (
    <section className="ui-client-chart">
      <Breadcrumbs
        items={[
          { label: "Clients", href: `/practice/${organizationId}/clients` },
          { label: name },
        ]}
      />
      <PageHeader
      ...
      {tab === "meal-plan" ? (
        <Section
          title="Meal plans"
          actions={
            <Link href={`/practice/${organizationId}/meal-plans`} className="ui-btn ui-btn--secondary ui-btn--sm">
              All meal plans
            </Link>
          }
        >
          {plans.length === 0 ? (
            <EmptyState
              ...
              action={
                <Link href={`/practice/${organizationId}/meal-plans`} className="ui-btn ui-btn--primary">
                  Open meal plans
                </Link>
              }
            />
            ...
            <ul className="ui-client-chart__list">
              {plans.map((plan) => (
                <li key={plan.id}>
                  <Link href={`/practice/${organizationId}/meal-plans/${plan.id}`} className="ui-link" style={{ fontWeight: 500 }}>
                    {plan.name}
                  </Link>
            ...
            <Table>
              ...
              <tbody>
                {clientInvoices.map((row) => (
                  <tr key={row.id}>
                    <Td label="Invoice">
                      <Link href={`/practice/${organizationId}/invoices/${row.id}`} className="ui-link">
