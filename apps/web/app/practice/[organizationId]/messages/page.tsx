...
export default function OrgMessagesPage() {
  ...
  useEffect(() => {
    ...
    setMessages(null);
    setThreadError(null);
    const base = `/api/v1/organizations/${organizationId}/clients/${selectedClientId}`;
    void api<ChatMessage[]>(`${base}/conversation/messages`)
      .then(async (data) => {
        ...
        const inbox = await api<InboxRow[]>(`/api/v1/organizations/${organizationId}/conversations`).catch(() => null);
  ...
  async function onSend(event: FormEvent) {
    ...
    try {
      ...
      setRows(await api<InboxRow[]>(`/api/v1/organizations/${organizationId}/conversations`));
    } catch (err) {
              ...
              <>
                <div className="ui-practice-messages__thread-head">
                  <div>
                    ...
                    <Link
                      href={`/practice/${organizationId}/clients/${selected.clientId}?tab=messages`}
                      className="ui-link"
                    >
