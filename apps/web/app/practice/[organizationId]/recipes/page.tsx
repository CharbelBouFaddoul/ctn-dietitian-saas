  ...
  return (
    <section>
      <PageHeader
        ...
        actions={
          <Link href={`/practice/${organizationId}/recipes/new`} className="ui-btn ui-btn--secondary">
            New recipe
          </Link>
        }
      />
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          action={
            !q ? (
              <Link href={`/practice/${organizationId}/recipes/new`} className="ui-btn ui-btn--primary">
                Create first recipe
              </Link>
            ) : undefined
          }
        ...
        <Table>
          ...
          <tbody>
            {items.map((row) => (
              <tr key={row.id}>
                <Td label="Recipe">
                  <Link href={`/practice/${organizationId}/recipes/${row.id}`} className="ui-link">
                    {row.name}
                  </Link>
                </Td>
