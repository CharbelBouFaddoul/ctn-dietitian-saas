...
export default function RecipeDetailPage() {
  ...
  return (
    <section>
      <Breadcrumbs
        items={[
          { href: `/practice/${organizationId}/recipes`, label: "Recipes" },
          { label: recipe.name },
        ]}
      />
        ...
        actions={
          <>
            <Button
              ...
              onClick={() =>
                void api<{ id: string }>(
                  `/api/v1/organizations/${organizationId}/recipes/${recipeId}/duplicate`,
                  { method: "POST" },
                ).then((copy) => {
                  window.location.href = `/practice/${organizationId}/recipes/${copy.id}`;
                })
              }
            >
              Duplicate
            ...
            {recipe.status === "ACTIVE" ? (
              <Button
                ...
                onClick={() =>
                  void api(
                    `/api/v1/organizations/${organizationId}/recipes/${recipeId}/archive`,
                    { method: "POST" },
                  ).then(() => load())
                }
              >
