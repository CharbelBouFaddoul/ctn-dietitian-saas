...
export default function ClientDocumentsPage() {
  ...
  useEffect(() => {
    void load().catch((err) => setError(err instanceof Error ? err.message : "Unable to load documents"));
  }, []);
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? ""}/api/v1/portal/documents`, {
        method: "POST",
        body,
        credentials: "include",
      });
  ...
  function downloadUrl(id: string) {
    ...
    return `${base}/api/v1/portal/documents/${id}/download`;
  }
