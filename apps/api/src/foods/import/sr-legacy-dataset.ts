...
function convertFood(food: FdcFood): FoodDatasetRecord | null {
...
export function datasetFromSrLegacyDump(dumpPath: string): FoodDatasetFile {
  ...
  const foods = raw.SRLegacyFoods ?? raw.SrLegacyFoods ?? raw.foods;
  if (!Array.isArray(foods)) {
    throw new Error("SR Legacy dump missing SRLegacyFoods[]");
  }
  const converted: FoodDatasetRecord[] = [];
  for (const food of foods) {
    const row = convertFood(food);
    if (row) converted.push(row);
  }
  return { source: { ...SR_LEGACY_SOURCE }, foods: converted };
}
