...
- Phase 6 client portfolio: `GET …/clients/:clientId/portfolio` composes identity, profile, latest measurements/BMI, goals, assessment, meal plan, appointment, messages, small recent timeline, and missing/alerts. Portal profile (`GET`/`PATCH /portal/me`) lets the patient edit identity and emergency contact for the active connection; allergies, lifestyle, and goals stay clinic-owned (read-only on the portal). Patients request visits as `REQUESTED`; the clinic accepts (`SCHEDULED`) or declines (`CANCELLED`).
