...
**Assessments V1 hardening (done):** `validateAssessmentResponses` on PATCH/complete (required on submit; type/option checks); completed assessments immutable; focused `assessment.e2e.spec.ts` (validation, snapshot freeze, forged portal IDs, multi-practice); Patient Evaluation Form UX (preview, progress, Save & continue, submit confirm); docs clarify Template vs Assessment vs snapshot. No migration. No portal self-start. No DATE type / SUBMITTED enum.

**Product Phase 8 (done):** Curated Foundation food catalog (`usda-foundation-curated.json` + `pnpm food:import` via `tsx` / admin import); `Food.dietitianAccountId` for practice-private custom foods; search `origin` + prefix ranking; Recipes as reusable meal library including platform **Starter** recipes (`dietitianAccountId` null, `pnpm recipe:import`); portal foods search includes practice-visible foods for the active connection (Phase 10); nutrition still via `packages/nutrition`. `Meal` remains a meal-plan day structure only. Meal-plan weeks are UI/presentation derived from `dayNumber`.
...
| Log | Portal write | Practice read | Uniqueness |
|---|---|---|---|
| Food | ✓ | ✓ | — |
| Water | ✓ | ✓ | — |
| Exercise | ✓ | ✓ | — |
| Sleep | ✓ | ✓ | `(dietitianAccountId, client, date)` |
| Habits | ✓ | ✓ | `(dietitianAccountId, client, habitKey, logDate)` |
| Measurements | ✓ (Phase 10 portal POST) | ✓ | — |

- Patient creates via `/api/v1/portal/tracking/*` (+ `POST /portal/measurements`).
...
20. **Payment collection:** Stay manual “mark paid”, or integrate a PSP?
21. **Product email default:** ✅ Phase 5 — `emailNotificationsEnabled` defaults false; admin toggle; auth emails unaffected.
